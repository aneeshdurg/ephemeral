...
document.addEventListener('DOMContentLoaded', () => {
    ...
    class UIElements {
        ...
        constructor() {
            ...
            const postInput = <HTMLInputElement>document.getElementById("post-input")!;
        ...
        updateConnectionsUI() {
            this.totalConnections.innerHTML = "" + (potentialPeers.size +
                                                    currentConnections);
    ...
    async function setupIdentity(id: string) {
        ...
        if (idmgmt == "createid") {
            ...
            const keys = <CryptoKeyPair> await crypto.subtle.generateKey(keyParams, true, ["sign", "verify"]);
            ...
            const globalID = await digestMessage(<string> pubKeyJWK.n);
        ...
        if (idmgmt != "guest") {
         ...
         ui.logToConsole(`Registering on network as guest:<br><b>${name}</b>@${id}`);
